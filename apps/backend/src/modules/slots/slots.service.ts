...
function validarDia(iso: string): void {
  const d = new Date(`${iso}T12:00:00`);
  const dow = d.getDay(); // 6=sábado
  if (dow !== 6) {
    throw new BadRequestException('A data deve ser um sábado');
  }
}
...
export class SlotsService {
  ...
  async create(dto: CreateSlotDto): Promise<Slot> {
    validarDia(dto.data); // valida dia da semana
    const exists = await this.repo.findOneBy({ data: dto.data });
    if (exists)
      throw new BadRequestException('Já existe um slot para esta data');
  ...
  // Validação de dia permitido (usado por outros serviços)
